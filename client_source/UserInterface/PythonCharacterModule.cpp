...
PyObject* chrLookAtDestPixelPosition(PyObject* poSelf, PyObject* poArgs)
{
	int pixelPosX;
	if (!PyTuple_GetInteger(poArgs, 0, &pixelPosX))
		return Py_BuildException();

	float pixelPosY;
	if (!PyTuple_GetFloat(poArgs, 1, &pixelPosY))
		return Py_BuildException();
	...
	pkInst->NEW_LookAtDestPixelPosition(TPixelPosition(pixelPosX, pixelPosY, 0.0f));
	return Py_BuildNone();
}

PyObject* chrIsWalking(PyObject* poSelf, PyObject* poArgs)
{
	int iVirtualID;
	if (!PyTuple_GetInteger(poArgs, 0, &iVirtualID))
		return Py_BuildException();
	...
	return Py_BuildValue("i", pInstance->IsGoing());
}
#endif
...
#ifdef ENABLE_NEW_CHARSELECT
		{ "LookAtDestPixelPosition",		chrLookAtDestPixelPosition,			METH_VARARGS },
		{ "IsWalking",						chrIsWalking,						METH_VARARGS },
#endif
		{ NULL,								NULL,								NULL		 },
	};
