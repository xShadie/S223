...
bool LoadLocaleData(const char* localePath)
{
	...
	char szItemName[256];
	char szItemNameCustom[256];
	char szMobName[256];
	char szMobNameCustom[256];

	snprintf(szItemList,	sizeof(szItemList) ,	"locale/general/item_list.txt");
	snprintf(szSkillTableFileName, sizeof(szSkillTableFileName), "locale/general/SkillTable.txt");
	...
	snprintf(szItemScale, sizeof(szItemScale), "locale/general/item_scale.txt");
#ifdef ENABLE_SHINING_SYSTEM
	snprintf(szShiningTable, sizeof(szShiningTable), "locale/general/shiningtable.txt");
#endif
	snprintf(szItemProto, sizeof(szItemProto), "locale/general/item_proto");
	snprintf(szMobProto, sizeof(szMobProto), "locale/general/mob_proto");
	snprintf(szInsultList, sizeof(szInsultList), "locale/general/insult.txt");
#ifdef ENABLE_MULTILANGUAGE
	...
	snprintf(szItemName, sizeof(szItemName), "%s/item_names.txt", localePath);
	snprintf(szItemNameCustom, sizeof(szItemNameCustom), "%s/item_names_custom.txt", localePath);
	snprintf(szMobName, sizeof(szMobName), "%s/mob_names.txt", localePath);
	snprintf(szMobNameCustom, sizeof(szMobNameCustom), "%s/mob_names_custom.txt", localePath);
	...
	if (!rkItemMgr.LoadItemName(szItemName))
	{
		Tracenf("LoadLocaleData - LoadItemName(%s) Error", szItemName);
		return false;
	}

	if (!rkItemMgr.LoadItemName(szItemNameCustom))
	{
		Tracenf("LoadLocaleData - LoadItemNameCustom(%s) Error", szItemNameCustom);
	}

#ifdef ENABLE_CHEST_DROP
	char szChestDrop[256];
	snprintf(szChestDrop, sizeof(szChestDrop), "locale/general/chest_drop");
	...
	if (!rkNPCMgr.LoadMobName(szMobName))
	{
		Tracenf("LoadLocaleData - LoadMobName(%s) Error", szMobName);
		return false;
	}

	if (!rkNPCMgr.LoadMobName(szMobNameCustom))
	{
		Tracenf("LoadLocaleData - LoadMobNameCustom(%s) Error", szMobNameCustom);
	}
