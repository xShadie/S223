...
void CInstanceBase::__GetShiningEffect(CItemData* pItem)
{
	...
	if (pItem->GetType() == CItemData::ITEM_TYPE_WEAPON)
	{
		...
		DWORD weaponType = pItem->GetType() == CItemData::ITEM_TYPE_COSTUME && pItem->GetSubType() == CItemData::COSTUME_WEAPON ? pItem->GetValue(3) : pItem->GetSubType();

		for (int i = 0; i < CItemData::ITEM_SHINING_MAX_COUNT; i++)
		{
			if (strcmp(shiningTable.szShinings[i], ""))
			{
				if (weaponType == CItemData::WEAPON_BOW)
				{
				...
				{
#ifdef ENABLE_WOLFMAN
					bool twoSidedWeapon = weaponType == CItemData::WEAPON_DAGGER || (IsMountingHorse() && weaponType == CItemData::WEAPON_FAN || weaponType == CItemData::WEAPON_CLAW);
#else
					bool twoSidedWeapon = weaponType == CItemData::WEAPON_DAGGER || (IsMountingHorse() && weaponType == CItemData::WEAPON_FAN);
#endif
					if (twoSidedWeapon)
					{
						__AttachWeaponShiningEffect(i, shiningTable.szShinings[i], "PART_WEAPON_LEFT");
