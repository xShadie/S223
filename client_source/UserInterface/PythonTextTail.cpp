...
void CPythonTextTail::RegisterCharacterTextTail(DWORD dwGuildID, DWORD dwVirtualID, const D3DXCOLOR & c_rColor, float fAddHeight)
{
	...
	TTextTail * pTextTail = RegisterTextTail(dwVirtualID,
											 ...
											 c_rColor
#ifdef ENABLE_LANGUAGE_FLAGS
											, pCharacterInstance->GetLanguageString()
#endif
	);
	...
	if (pCharacterInstance->IsPC())
	{
		...
		{
			char szPathLanguage[256];
			sprintf(szPathLanguage, "d:/ymir work/ui/flags/%s.tga", c_szLanguageString);
			if (CResourceManager::Instance().IsFileExist(szPathLanguage))
			{
				pTextTail->pLanguageInstance = CGraphicImageInstance::New();
				pTextTail->pLanguageInstance->SetImagePointer((CGraphicImage*)CResourceManager::Instance().GetResourcePointer(szPathLanguage));
			}
		}
	}
#endif
