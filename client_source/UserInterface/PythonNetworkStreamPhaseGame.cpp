...
bool CPythonNetworkStream::SendGuildChargeGSPPacket(long long lldMoney)
{
	TPacketCGGuild GuildPacket;
	GuildPacket.byHeader = HEADER_CG_GUILD;
	...
	if (!Send(sizeof(lldMoney), &lldMoney))
		return false;
...
bool CPythonNetworkStream::SendGuildDepositMoneyPacket(long long lldMoney)
{
	TPacketCGGuild GuildPacket;
	GuildPacket.byHeader = HEADER_CG_GUILD;
...
bool CPythonNetworkStream::SendGuildWithdrawMoneyPacket(long long lldMoney)
{
	TPacketCGGuild GuildPacket;
	GuildPacket.byHeader = HEADER_CG_GUILD;
...
bool CPythonNetworkStream::RecvWikiPacket()
{
	...
	_wGC_PACKAGE_SIZE iPacketSize = pack.size - sizeof(InGameWiki::TGCWikiPacket);
	if (iPacketSize <= 0)
		return false;

	_wuint64 ret_id = 0;
	_wuint32 data_vnum = 0;

	if (pack.is_data_type(InGameWiki::LOAD_WIKI_ITEM))
	{
		const size_t recv_size = sizeof(InGameWiki::TGCItemWikiPacket);
		iPacketSize -= _wGC_PACKAGE_SIZE(recv_size);
		...
		const _wint32 origin_size = item_data.origin_infos_count;
		const _wint32 chest_info_count = recv_wiki.chest_info_count;
		const _wint32 refine_infos_count = recv_wiki.refine_infos_count;
...

		{
			...
			for (_wint32 idx = 0; idx < origin_size; ++idx) {
				...
				iPacketSize -= _wGC_PACKAGE_SIZE(origin_info_recv_base_size);
			}
			...
			for (_wint32 idx = 0; idx < chest_info_count; ++idx) {
			...
			for (_wint32 idx = 0; idx < refine_infos_count; ++idx) {
				...
				iPacketSize -= _wGC_PACKAGE_SIZE(refine_info_recv_base_size);
			}
		}
	...
	else
	{
		...
		const _wint32 drop_info_count = mob_data.drop_info_count;
...

		{
			...
			for (_wint32 idx = 0; idx < drop_info_count; ++idx) {
				...
				iPacketSize -= _wGC_PACKAGE_SIZE(mob_drop_info_recv_base_size);
			}
		}
