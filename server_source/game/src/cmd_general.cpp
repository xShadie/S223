...
ACMD(do_user_horse_ride)
{
	...
	if (ch->IsHorseRiding() == false)
	{
		if (ch->GetMountVnum())
		{
			ch->ChatPacket(CHAT_TYPE_INFO, "[LS;1323]");
			return;
		}
...
ACMD(do_user_horse_feed)
{
	...
	if (ch->CountSpecifyItem(dwFood) > 0)
	{
		...
		ch->FeedHorse();
		ch->ChatPacket(CHAT_TYPE_INFO, "[LS;505;%s;%s]", ITEM_MANAGER::instance().GetTable(dwFood)->szLocaleName, "");
	}
	else
	{
...
ACMD(do_restart)
{
	...
	if (subcmd == SCMD_RESTART_TOWN)
	{
		...
		if (iTimeToDead > 173)
		{
			ch->ChatPacket(CHAT_TYPE_INFO, "[LS;516;%d]", iTimeToDead - 173);
			return;
		}
	}
...
ACMD(do_messenger_auth)
{
	...
	if (answer != 'y')
	{
		...
		if (tch)
			tch->ChatPacket(CHAT_TYPE_INFO, "[LS;657]");
	}
}
...
ACMD(do_unmount)
{
	...
	{
		ch->ChatPacket(CHAT_TYPE_INFO, "[LS;1229]");
	}
...
ACMD(do_party_request)
{
	if (ch->GetParty())
	{
		ch->ChatPacket(CHAT_TYPE_INFO, "[LS;1524]");
		return;
	}
