			...
			if (0!=processReturn)
			{
				if (ch->GetDesc())
				{
					...
					if (pTable)
					{
						...
						int len;
						if (3==processReturn)
							len = snprintf(buf, sizeof(buf), "[LS;841]");
						else
							len = snprintf(buf, sizeof(buf), "[LS;507;[IN;%d]]", ITEM_PRISM);
...
int CInputMain::Chat(LPCHARACTER ch, const char * data, size_t uiBytes)
{
	...
	if (0!=processReturn)
	{
		...
		{
			if (3==processReturn)
				ch->ChatPacket(CHAT_TYPE_INFO, "[LS;841]");
			else
				ch->ChatPacket(CHAT_TYPE_INFO, "[LS;507;%d]", ITEM_PRISM);
		}
...
void CInputMain::Exchange(LPCHARACTER ch, const char * data)
{
	...
	if ((to_ch = CHARACTER_MANAGER::instance().Find(pinfo->arg1)))
	{
		...
		{
			to_ch->ChatPacket(CHAT_TYPE_INFO, "[LS;528]");
			return;
		}
...
void CInputMain::SafeboxCheckin(LPCHARACTER ch, const char * c_pData)
{
	...
	if (pkItem->GetType() == ITEM_BELT && pkItem->IsEquipped())
	{
		...
		ch->ChatPacket(CHAT_TYPE_INFO, "[LS;147]");
		return;
	}
...
void CInputMain::RecvWikiPacket(LPCHARACTER ch, const char* c_pData)
{
	...
	pack.increment_data_size(_wGC_PACKAGE_SIZE(sizeof(InGameWiki::TGCWikiPacket)));
	...
	if (pack.is_data_type(InGameWiki::LOAD_WIKI_ITEM))
	{
		...
		pack.increment_data_size(_wGC_PACKAGE_SIZE(buf_data_dize));
		...
		{
			if (origin_size)
				for (_wint32 idx = 0; idx < (_wint32)origin_size; ++idx)
					buf.write(&(originVec[idx]), sizeof(CommonWikiData::TWikiItemOriginInfo));

			if (chest_info_count > 0) {
				for (_wint32 idx = 0; idx < (_wint32)chest_info_count; ++idx) {
			...
			if (refine_infos_count > 0)
				for (_wint32 idx = 0; idx < (_wint32)refine_infos_count; ++idx)
	...
	else
	{
		...
		{
			if (_mobVec_size) {
				for (_wint32 idx = 0; idx < (_wint32)_mobVec_size; ++idx) {
...
int CInputMain::Guild(LPCHARACTER ch, const char * data, size_t uiBytes)
{
	...
	if (NULL == pGuild)
	{
		...
		{
			ch->ChatPacket(CHAT_TYPE_INFO, "<Guild> It does not belong to the Guild.");
			return SubPacketLen;
		}
	}
		...
		case GUILD_SUBHEADER_CG_OFFER:
			{
				...
				{
					ch->ChatPacket(CHAT_TYPE_INFO, "[LS;1052]");
				}
				else
				{
					offer /= 100;
