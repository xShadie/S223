import app, item, player, nonplayer, ui, wndMgr, grp, math, wiki, ingamewikiconfig,\
 uiToolTip, types, constInfo, uiScriptLocale, dbg, sys, chat
...
class WikiCategory(ui.Window):
	...
	def __init__(self, owner = None):
		...
		self.titleImg.SetStringEvent("MOUSE_LEFT_DOWN",self.ClickExpand)
		self.titleImg.Show()
...
class WikiMainWeaponWindow(ui.Window):
	class WikiItem(ui.Window):
		...
		def __init__(self, vnum, parent, allow_special_page = True):
			...
			self.itemImage.Show()
			self.itemImage.SetStringEvent("MOUSE_OVER_IN", self.parent.OnOverIn, self.vnum + (9 if wiki.CanIncrRefineLevel() else 0))
			self.itemImage.SetStringEvent("MOUSE_OVER_OUT", self.parent.OnOverOut)
			if wiki.GetBaseClass() and allow_special_page:
				self.itemImage.SetStringEvent("MOUSE_LEFT_DOWN", wiki.GetBaseClass().OpenSpecialPage, self.parent, self.vnum, False)
			...
			for i in xrange(wiki.MAX_REFINE_COUNT):
				...
				for j in retInfo[(i+starting_col)][1]:
					...
					if j[0] != 0:
						...
						currWindow.refineMat[curr].SetStringEvent("MOUSE_OVER_IN", self.parent.OnOverIn, j[0])
						currWindow.refineMat[curr].SetStringEvent("MOUSE_OVER_OUT", self.parent.OnOverOut)
		...
		def NoticeMe(self, noticeType = [False, 0]):
			...
			self.itemImage.SetStringEvent("MOUSE_OVER_IN",self.parent.OnOverIn, self.vnum + (max_refine_count if wiki.CanIncrRefineLevel() else 0))
			self.itemImage.SetStringEvent("MOUSE_OVER_OUT", self.parent.OnOverOut)
		...
		if wiki.GetBaseClass():
			...
			if vnumEvent != -1:
				if not self.sendParent:
					tmp.SetStringEvent("MOUSE_LEFT_DOWN", wiki.GetBaseClass().OpenSpecialPage, None, vnumEvent, False)
				else:
					if hasattr(self.parent, "parent"):
						tmp.SetStringEvent("MOUSE_LEFT_DOWN", wiki.GetBaseClass().OpenSpecialPage, self.parent.parent, vnumEvent, False)
					else:
						tmp.SetStringEvent("MOUSE_LEFT_DOWN", wiki.GetBaseClass().OpenSpecialPage, self.parent, vnumEvent, False)
...
class WikiMainChestWindow(ui.Window):
	class WikiItem(ui.Window):
		def __init__(self, vnum, parent):
			...
			self.chestImage.SetStringEvent("MOUSE_OVER_IN", self.parent.OnOverIn, self.vnum)
			self.chestImage.SetStringEvent("MOUSE_OVER_OUT", self.parent.OnOverOut)
...
class SpecialPageWindow(ui.Window):
	...
	def __init__(self, vnum, isMonster):
		...
		if isMonster:
			...
			self.modelView.SetStringEvent("MOUSE_OVER_IN", self.OnOverIn, self.vnum + (wiki.MAX_REFINE_COUNT if wiki.CanIncrRefineLevel() else 0))
			self.modelView.SetStringEvent("MOUSE_OVER_OUT", self.OnOverOut)
			self.modelView.Show()
...
class WikiMainCostumeWindow(ui.Window):
	class WikiItem(ui.Window):
		def __init__(self, vnum, parent):
			...
			self.costumeImage.SetStringEvent("MOUSE_OVER_IN", self.parent.OnOverIn, self.vnum)
			self.costumeImage.SetStringEvent("MOUSE_OVER_OUT", self.parent.OnOverOut)
...
def BuildBaseMain(self):
	...
	self.header.SetStringEvent("MOUSE_LEFT_DOWN", self.GoToLanding)
