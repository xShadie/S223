...
class Window(object):
	...
	def __init__(self, layer = "UI"):
		...
		self.onMouseLeftButtonUpArgs = None
	...
	def SetGeraldEvent(self, event, *args):
	...
	def OnMouseLeftButtonUp(self):
		...
		if self.onMouseLeftButtonUpEvent:
			if self.onMouseLeftButtonUpArgs:
				self.onMouseLeftButtonUpEvent(*self.onMouseLeftButtonUpArgs)
			else:
...
class ImageBox(Window):
	def __init__(self, layer = "UI"):
		...
		# To fix mouse left button overload
	...
	def OnMouseOverIn(self):
		if not isinstance(self.eventArgs, dict) and len(self.eventDict) != 0:
			self.eventDict["MOUSE_OVER_IN"](*self.eventDict["MOUSE_OVER_IN"])
		elif not isinstance(self.eventArgs, dict) and len(self.eventDict2) != 0:
			self.eventDict2["MOUSE_OVER_IN"][0](*self.eventDict2["MOUSE_OVER_IN"][1])
		elif self.eventFunc["mouse_over_in"]:
			self.eventFunc["mouse_over_in"](*self.eventArgs["mouse_over_in"])
		else:
			try:
				self.eventDict["MOUSE_OVER_IN"](*self.argDict["MOUSE_OVER_IN"])
			except KeyError:
				pass

	def OnMouseOverOut(self):
		if not isinstance(self.eventArgs, dict) and len(self.eventDict) != 0:
			self.eventDict["MOUSE_OVER_OUT"](*self.eventDict["MOUSE_OVER_OUT"])
		elif not isinstance(self.eventArgs, dict) and len(self.eventDict2) != 0:
			self.eventDict2["MOUSE_OVER_OUT"][0](*self.eventDict2["MOUSE_OVER_OUT"][1])
		elif self.eventFunc["mouse_over_out"]:
			self.eventFunc["mouse_over_out"](*self.eventArgs["mouse_over_out"])
		else:
			try:
				self.eventDict["MOUSE_OVER_OUT"](*self.argDict["MOUSE_OVER_OUT"])
			except KeyError:
				pass
	...
	def OnMouseLeftButtonUp(self):
		if "MOUSE_LEFT_UP" in self.eventDict:
			self.eventDict["MOUSE_LEFT_UP"](*self.argDict["MOUSE_LEFT_UP"])

		if self.eventFunc:
			if isinstance(self.eventFunc, dict) and self.eventFunc["mouse_click"]:
				self.eventFunc["mouse_click"](*self.eventArgs["mouse_click"])
	...
	# Overloading window methods which is not used in case of imagebox or children class
