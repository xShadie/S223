...
class ItemToolTip(ToolTip):
	...
	def AppendPrice(self, price):	
		self.AppendSpace(5)
		self.AppendTextLine(localeInfo.TOOLTIP_BUYPRICE  % (localeInfo.NumberToMoneyStringOld(price)), self.GetPriceColor(price))
	...
	def AppendSellingPrice(self, price):
		if item.IsAntiFlag(item.ITEM_ANTIFLAG_SELL):			
			...
			self.AppendTextLine(localeInfo.TOOLTIP_SELLPRICE % (localeInfo.NumberToMoneyStringOld(price)), self.GetPriceColor(price))
			self.AppendSpace(5)
